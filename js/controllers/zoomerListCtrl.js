...
.controller('zoomerListCtrl', ['$state', '$scope', '$timeout', '$interval', '$http', '$uibModal', '$q', '$rootScope', '$location', function($state, $scope, $timeout, $interval, $http, $uibModal, $q, $rootScope, $location){
   ...
   // $scope.vehicle=[];
    ...
    $scope.gridOptions = {
        ...
        columnDefs:[
            { field: 'zoomerid', displayName : 'Delivery Agent ID'  },
            ...
            { field: 'v_vehicle', displayName : 'Vehicle' },
    ...
    /**/
/*$scope.getVehiclesType = function(){
    $http.get('http://18.141.18.7/controlpanel/getVehiclesType.php')        
    .success(function(respone) 
    {
        respone = CRYPTO.decrypt(respone.data);
         for(var i=0, length=respone.result.length;i<length;i++){
                respone.result[i].vehicle_id= decodeURIComponent(respone.result[i].vehicleid) ;
                respone.result[i].vehicles_desc= decodeURIComponent(respone.result[i].vehiclesdesc) ;
        }
    ...
    /*$scope.changeDescription = function(){
        for(var i=0, length=$scope.vehicle.length;i<length;i++){
            if($scope.input.vehicle_id == $scope.vehicle[i].vehicleid){
                $scope.vehicles_desc = $scope.vehicle[i].vehiclesdesc;
                break;
            }
        }
    ...
    $scope.addZoomer = function(){
        var param = {
            ...
            vehicles_id:'',
            type: '1',
            //vehiclesTypelist : $scope.vehiclesTypelist,
            //vehicle_id:'',
...
.controller('ZoomerModalCtrl', ['$scope', '$uibModalInstance', 'stateParam', '$http', '$uibModal','Upload', function ($scope, $uibModalInstance, stateParam, $http, $uibModal, Upload) {
   ...
   $scope.init = function(){
        $scope.getVehicles();
           ...
           // $scope.input.vehicle_id = $scope.dataParam.vehicleid; 
    ...
    $scope.editForm = function (files) {
		if(files) 
		{
			files.upload = Upload.upload({ 
				  url: 'http://18.141.18.7/controlpanel/editZoomer.php',
				  method: 'POST',
				  data: {'file':files
    ...
    $scope.submit = function(){
        ...
        var data = {};
        var url = 'http://18.141.18.7/controlpanel/addZoomer.php';
