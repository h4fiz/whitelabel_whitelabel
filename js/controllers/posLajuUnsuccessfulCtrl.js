...
.controller('posLajuUnsuccessfulCtrl', ['$state', '$scope', '$timeout', '$interval', '$http', '$uibModal', '$q', '$rootScope', '$window', '$location',  function($state, $scope, $timeout, $interval, $http, $uibModal, $q, $rootScope, $window, $location, $uibModalInstance){
    ...
    $scope.gridOptions = {
        ...
        columnDefs:[
            { displayName: '', field: 'isPicked', width:40, pinnedLeft:true, cellTemplate: '<input type="checkbox" ng-checked="row.entity.isPicked" ng-click="grid.appScope.pick(row.entity)">'},
...
*/
    ...
    $scope.iniCancelAll = function (rawData) {
        var modalReset = $uibModal.open({
          animation: true,
          templateUrl: 'iniCancelAll.html',
          controller: 'iniCancelAllCtrl',
          size: 'sm',
          resolve: {
            selectedRow: rawData
            }
    ...
    // console.log(array);
        ...
        $scope.cancelOrderAll = function() {  
            if ( $scope.dataed.Password == "zoomitnow3368" ) {
                ...
                $http({
                    method: 'POST',
                    url:'http://18.141.18.7/controlpanel/updatePosLajuUnsuccessfulSelected.php',data:{'data' : jsonData},  
                    headers: {'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'},}).success(function (respone, status, headers, config) 
                    {
...
angular.module('app').controller('iniCancelAllCtrl', function ($scope, $uibModalInstance, $http, $timeout, selectedRow, Upload) {
