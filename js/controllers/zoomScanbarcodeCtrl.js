...
.controller('zoomScanbarcodeCtrl', ['$state', '$scope', '$timeout', '$interval', '$http', '$uibModal', '$q', '$rootScope', '$location', function($state, $scope, $timeout, $interval, $http, $uibModal, $q, $rootScope, $location){
    ...
    //     var samedatabase = true;
    //     for(var i =0; i< $scope.barcode.length;i++){
    //         if($scope.barcode[i].barcode == $scope.input.barcode){
    //             samedatabase = false;
    //             break;
    //         }
    //     }
    ...
    //         $scope.list.push({ id:$scope.list.length, barcode: $scope.input.barcode, n_partnerid: $scope.input.n_partnerid });    
    //     }else {
    //         alert('This barcode already on delivery');
    //     }
    ...
    $scope.addBarcode = function(){
        ...
        if(!same){
            ...
            if(!samedatabase){
                ...
                alert('This barcode already on progres!');
            }
            // $scope.list.push({ id:$scope.list.length, barcode: $scope.input.barcode, n_partnerid: $scope.input.n_partnerid });    
        }else {
            alert('This barcode already scanned!');
    ...
    {
        $http.get('http://'+$location.$$host+'/controlpanel/GetBarcode.php')        
        .success(function(respone) 
        {
            respone = CRYPTO.decrypt(respone.data);

            $scope.barcode = respone.records;
        ...
        .error(function () 
        {
            console.log('Unable to get Barcode' );
        });
    };
