...
.controller('orderReservationCtrl', ['$state', '$scope', '$timeout', '$interval', '$http', '$uibModal', '$q', '$rootScope', '$window', function($state, $scope, $timeout, $interval, $http, $uibModal, $q, $rootScope, $window){
    ...
    $scope.gridOptions = {
        ...
        columnDefs:[
            { field: 'zoomorderid', displayName : 'ID', width:100,cellTemplate:'<div class="ui-grid-cell-contents"><button class="btn btn-primary btn-xs" ng-click="grid.appScope.openDetailOrderModal(row.entity.zoomorderid)">+</button>&nbsp;&nbsp;{{ row.entity.zoomorderid }}</div>' },
            ...
            { field: 'Delivery_Instruction', displayName:'Delivery Instruction', width:200
                /*filter: {
                    term: '',
            ...
            { name: 'actions', enableFiltering:false, field:'Ticketid', width:150, pinnedRight: true, cellTemplate:'<button class="btn btn-primary btn-sm" ng-click="grid.appScope.openAssignOrderModal(row.entity)">Assign</button><button class="btn btn-warning btn-sm" ng-click="grid.appScope.cancelModal(row.entity)">Delete</button>'}
        ],
    };
...
// } 
    ...
    $scope.cancelModal = function (rawData) {
        var modalReset = $uibModal.open({
          animation: true,
          templateUrl: 'insertPaswordOrderReservation.html',
          controller: 'insertPaswordOrderReservationCtrl',
          size: 'sm',
          resolve: {
            selectedRow: rawData
            }
...
angular.module('app').controller('insertPaswordOrderReservationCtrl', function ($scope, $uibModalInstance, $http, $timeout, selectedRow, Upload) {
    $scope.dataed =[];
        $scope.dataed.zoomorderid=selectedRow.zoomorderid;
        $scope.dataed.driverID=selectedRow.driverid;
    ...
    $scope.cancelOrder = function(data) {   
            ...
            // if ($scope.dataed.Password == "zoomitnow3368") {
                var dataParameter = { 
                    'zoomID': $scope.dataed.zoomorderid,
                    'driverID': $scope.dataed.driverID
                };
                var jsonData = CRYPTO.encrypt(dataParameter);
                $http({method: 'POST',
                url:'http://18.141.18.7/controlpanel/updatePosLajuCancelOrderSubmited.php', 
                data:{'data' : jsonData},  
                headers: {'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'},}).success(function (response) 
                // $http.get('http://18.141.18.7/controlpanel/updatePosLajuCancelOrderSubmited.php?data='+$scope.data)      
                // .success(function(response) 
                { 
