angular.module('app')
.controller('zoomerFreelanceCtrl', ['$state', '$scope', '$timeout', '$interval', '$http', '$uibModal', '$q', '$rootScope', '$location', function($state, $scope, $timeout, $interval, $http, $uibModal, $q, $rootScope, $location){

    /*----------*//*-----VARIABLES-----*//*----------*/
    $scope.list = [];
    $scope.TypeName = "Freelancer";
    ...
    $scope.getList = function(){
        $http.get('http://'+$location.$$host+'/controlpanel/getZoomerList.php?type=2')		
		.success(function(respone) 
		{
            respone = CRYPTO.decrypt(respone.data);
			 for(var i=0, length=respone.result.length;i<length;i++){
...
angular.module('app')
.controller('ZoomerFreelanceModalCtrl', ['$scope', '$uibModalInstance', 'stateParam', '$http', '$uibModal','Upload', '$location', function ($scope, $uibModalInstance, stateParam, $http, $uibModal, Upload, $location) {
